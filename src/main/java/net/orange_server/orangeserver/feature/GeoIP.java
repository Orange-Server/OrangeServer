...
public class GeoIP {
     ...
     * @param plugin
     */
    public GeoIP(final OrangeServer plugin){
        instance = this;
        this.plugin = plugin;
    }
    ...
    public void init(){
        ...
        final ConfigurationManager conf = OSHelper.getInstance().getConfig();
    ...
    public String getGeoIpString(final InetAddress addr, final boolean simple){
        ...
        if (simple){
            if (OSHelper.getInstance().getConfig().getUseCityDB()){
        ...
        else if (OSHelper.getInstance().getConfig().getUseCityDB()){
